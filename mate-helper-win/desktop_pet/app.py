...
from desktop_pet.tools import screenshot as _screenshot_fn, listen as _listen_fn
...
class TetoPet(Gtk.Window):
    ...
    def __init__(self):
        ...
        self._ptt_socket_path = platform.ptt_socket_path()
        self._ptt_helper_path = platform.ptt_helper_path()
    ...
    def _start_ptt_socket_server(self):
        """Start a UNIX socket (Linux) or TCP socket (Windows) server for PTT commands."""
        try:
            if platform.IS_WINDOWS:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.bind(("127.0.0.1", 13777))
            else:
    ...
    def _create_ptt_helper(self):
        """Write a helper script that sends commands to the PTT UNIX socket.

        On Linux writes a bash script; on Windows writes a PowerShell script.
        Used by desktop environment global shortcuts."""
        ...
        if platform.IS_WINDOWS:
            helper = (
                ...
                "param([string]$cmd = 'toggle')\n"
                f'$sock = "\\\\.\\pipe\\mate-helper-ptt"\n'
                "try {\n"
                "    $client = New-Object System.Net.Sockets.TcpClient\n"
                "    $client.Connect('127.0.0.1', 13777)\n"
...
"""
    ...
    def _on_destroy(self, _w=None):
        ...
        if platform.IS_WINDOWS:
            try:
                import ctypes
                ctypes.windll.kernel32.CancelIoEx(0, 0)
            except Exception:
                pass
        self._stop_alarm_check()
        self._stop_alarm()
        platform.kill_process("parec.exe" if platform.IS_WINDOWS else "parec")
        if platform.IS_WINDOWS:
            subprocess.run(
